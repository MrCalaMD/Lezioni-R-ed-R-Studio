...
n = 500 #Numerosità
x <- rnorm(n = n, mean = 10, sd = 15) #Tipo di distribuzione
```
...
In particolare, la cosiddetta *k-fold* consiste nella suddivisione dell'insieme di dati totale in k parti di uguale numerosità e, a ogni passo, la kª parte dell'insieme di dati viene a essere quella di convalida, mentre la restante parte costituisce sempre l'insieme di addestramento.
...
sim_CV <- add_column(simulazione,
           gruppo = sample(rep(1:k,
                          ...
                          replace = F),
           .after = 2 )
#Verifico che ogni gruppo abbia la stessa numerosità
sim_CV %>% 
  group_by(gruppo) %>% 
  count() %>% 
  head(3)
...
Ora valuto la qualità dei modelli attraverso il calcolo del Mean Standard Error:<br><br>

$MSE = \frac{1}{n}\sum_{i=1}^{n}(Obs_{i}-Pred_{i})^2$<br><br>
Più basso è il MSE migliore è la qualità del modello che abbiamo costruito<br>
